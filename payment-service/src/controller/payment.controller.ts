...
import { ApiResponseHelper, authMiddleware, AuthenticatedRequest, NotFoundError } from 'shared';
...
export class PaymentController {
  ...
  private async process(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      res.status(201).json(ApiResponseHelper.success(responsePayload, 'Payment processed successfully'));
    } catch (error: any) {
      next(error);
    }
  }
  ...
  private async getById(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      res.json(ApiResponseHelper.success(payment));
    } catch (error: any) {
      next(error);
    }
  }
  ...
  private async getByOrder(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      res.json(ApiResponseHelper.success(payments));
    } catch (error: any) {
      next(error);
    }
  }
  ...
  private async refund(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      res.json(ApiResponseHelper.success(updatedPayment, 'Payment refunded successfully'));
    } catch (error: any) {
      next(error);
    }
  }
